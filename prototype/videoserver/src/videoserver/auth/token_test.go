...
func CheckTokenFields(t *testing.T, tok *Token, uri, consumer, lifetime, sig string) {
...
func CheckTokenFieldsWithTime(t *testing.T, tok *Token, uri, consumer string, lifetime, issuetime int64, sig string) {
...
func FakeToken(uri, consumer, valid, sig string) *Token {
	tok := Token{
		Resource:    uri,
		Valid_until: valid,
		Signature:   sig,
...
func TestTokenGeneration(t *testing.T) {
	...
	// create signed token
	tok, err := GenerateToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME, TEST_TOKEN_SECRET)
...
func TestTokenGenerationFailing(t *testing.T) {

	// test token generation with wrong input parameters
	// - empty resource
	_, err := GenerateToken("", TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME, TEST_TOKEN_SECRET)
	if err == nil {
		t.Error("token created when url was empty")
	}

	// - malformed resource (no url)
	_, err = GenerateToken("http/nourl", TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME, TEST_TOKEN_SECRET)
	if err == nil {
		t.Error("token created when url was malformed")
	}

	// empty consumer
	_, err = GenerateToken(TEST_TOKEN_URI, "", TEST_TOKEN_LIFETIME, TEST_TOKEN_SECRET)
	if err == nil {
		t.Error("token created when url was malformed")
	}

	// - empty signing key
	_, err = GenerateToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME, "")
	if err == nil {
		t.Error("token created when signing key was empty")
	}

	// - negative or zero validity time
	_, err = GenerateToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, time.Duration(-1), TEST_TOKEN_SECRET)
	if err == nil {
		t.Error("token created when lifetime was negative")
	}

	// - negative or zero validity time
	_, err = GenerateToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, time.Duration(0), TEST_TOKEN_SECRET)
	if err == nil {
...
func TestTokenInput(t *testing.T) {
	...
	r.Header.Add("Authorization", FakeToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME_STR, "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, _ := NewTokenFromHttpRequest(r)
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME_STR, "").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken("", TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME_STR, "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, "", "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken("http;//example.com", TEST_TOKEN_CONSUMER, TEST_TOKEN_LIFETIME_STR, "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, "10a", "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
	...
	r, _ = http.NewRequest("POST", TEST_TOKEN_URI, nil)
	r.Header.Add("Authorization", FakeToken(TEST_TOKEN_URI, TEST_TOKEN_CONSUMER, "-10", "sig").String())
	r.RemoteAddr = TEST_TOKEN_CONSUMER
	tok, err = NewTokenFromHttpRequest(r)
	if err == nil {
...
func TestTokenLifetimeVerification(t *testing.T) {

	// create fake token with valid lifetime
	future := time.Now().UTC().Unix() + 10
	tok_valid := FakeToken("", TEST_TOKEN_CONSUMER, strconv.FormatInt(future, 10), "")
	...
	past := future - 20
	tok_invalid := FakeToken("", TEST_TOKEN_CONSUMER, strconv.FormatInt(past, 10), "")
