...
public class FFmpegFrameSender extends com.googlecode.javacv.FrameRecorder {
    ...
    public void start() throws Exception {
        ...
        if (avcodec_open2(c, codec, opts) < 0) {
            Log.e("MSC", "Could not open codec");
        }
        
        this.picture = avcodec_alloc_frame();
        this.picture.format(c.pix_fmt());
        this.picture.width(c.width());
        this.picture.height(c.height());
        
        int[] temp = { this.picture.linesize().get(0), this.picture.linesize().get(1) };
       
        if (av_image_alloc(this.picture.data(), temp, c.width(), c.height(), c.pix_fmt(), 1) < 0) {
        	Log.e("MSC", "Could not allocate raw picture buffer");
        }
        */
